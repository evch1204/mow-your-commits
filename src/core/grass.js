// The five silhouettes, as drawing instructions rather than as drawing.
//
// `grassOps` returns a flat list of ops -- filled paths, strokes, ovals -- in
// board pixels. src/render2d replays them onto a canvas and src/export/svg.js
// serialises them into <path>s, so the picture in a README is the same drawing
// as the board on the page rather than a second implementation of it.
//
// Pure: no DOM, no dependencies, and every random number comes from the `rnd`
// handed in, so the canvas can boil its jitter every 8th of a second while the
// exporter re-seeds and gets the identical shape twice.

import { GEOM } from './board.js';
import { hash } from './lawn.js';
import {
  AUTUMN_BLADE, DANDELION, FLOWERS, FLUFF, LIGHT_RAMP,
  bladeColor, cellTint, grassFor, innerColor, mix,
} from './palette.js';

const { CELL } = GEOM;

/**
 * Paint order. Everything at one layer is drawn before anything at the next,
 * across a whole row, which is what lets the exporter batch a row's strokes by
 * colour and still come out looking like the canvas.
 */
export const Z = { SHADOW: 0, BODY: 1, DETAIL: 2, ACCENT: 3 };

/** How long a cut takes to squash the silhouette flat, matching MOW_ANIM. */
const FLAT = 0.15;      // the y scale a mown silhouette ends at
const OVERSHOOT = 0.15; // how far it bulges sideways on the way down

const easeOut = (t) => 1 - (1 - t) * (1 - t) * (1 - t);

/**
 * A quadratic that really passes through `(px, py)` at its midpoint: a plain
 * "control point at the peak" curve only reaches halfway there, which is what
 * made the first lobes look like dents.
 */
function apex(x0, y0, px, py, x1, y1) {
  return ['Q', 2 * px - (x0 + x1) / 2, 2 * py - (y0 + y1) / 2, x1, y1];
}

/**
 * The outline of a filled clump: a flat-ish base, a bowed side each end, and
 * `lobes` bumps across the top. Three lobes read as a bush, four as a hedge.
 */
function lobed(cx, base, halfW, h, lobes, peaks, w) {
  const left = cx - halfW;
  const right = cx + halfW;
  const shoulder = base - h * 0.3;
  const valley = base - h * 0.6;
  // the tops the curve has to pass through, valleys first then the peaks
  const tops = [];
  for (let k = 0; k <= lobes; k++) {
    const x = left + (k / lobes) * halfW * 2;
    const edge = k === 0 || k === lobes;
    tops.push([
      w(edge ? x + (k === 0 ? halfW * 0.06 : -halfW * 0.06) : x, 1),
      w(edge ? shoulder : valley, 1.4),
    ]);
  }

  const d = [['M', left, base]];
  d.push(['Q', left - halfW * 0.14, base - h * 0.24, tops[0][0], tops[0][1]]);
  for (let i = 0; i < lobes; i++) {
    const a = tops[i];
    const b = tops[i + 1];
    const px = (a[0] + b[0]) / 2 + (peaks[i][1] || 0);
    const py = base - h * peaks[i][0];
    d.push(apex(a[0], a[1], px, py, b[0], b[1]));
  }
  d.push(['Q', right + halfW * 0.14, base - h * 0.24, right, base]);
  d.push(['Z']);
  return { d, tops };
}

/** One blade, curving as it goes up. Returns the path and its tip. */
function blade(bx, base, h, lean, w) {
  const tip = [bx + lean, base - h];
  return {
    d: [
      ['M', w(bx, 0.5), base],
      ['Q', w(bx + lean * 0.35, 0.8), w(base - h * 0.62, 0.8), w(tip[0], 0.6), w(tip[1], 0.6)],
    ],
    tip,
  };
}

/**
 * Everything that grows on one day, in board pixels.
 *
 * @param cell   a lawn cell (only col/row/level/vigor/heroic are read)
 * @param x,y    the tile's top-left corner
 * @param season 0..3, or NO_SEASON for the plain chart
 * @param opts   { rnd, ramp, scale, mowT }
 *   rnd    () => [0,1); the caller owns the seed
 *   ramp   a theme (light by default), so dark mode follows every colour rule
 *   scale  1 on the board; the legend draws at its own size
 *   mowT   0 standing, 0..1 mid-cut, >= 1 gone
 * @returns [{ z, t, ... }] ops, in paint order within each layer
 */
export function grassOps(cell, x, y, season, opts = {}) {
  const level = cell.level;
  if (cell.void || !level) return [];
  const mowT = opts.mowT || 0;
  if (mowT >= 1) return [];             // cut: the clean GitHub tile is the point

  const rnd = opts.rnd || Math.random;
  const ramp = opts.ramp || LIGHT_RAMP;
  const scale = opts.scale == null ? 1 : opts.scale;
  const w = (v, a) => v + (rnd() - 0.5) * a;

  const g = grassFor(level, cell.vigor || 0);
  const cx = x + CELL / 2;
  const base = y + CELL - 1;
  const h = g.height * scale;
  const halfW = g.spread * scale;
  const fill = cellTint(bladeColor(level, season, ramp), cell.col, cell.row);
  const inner = innerColor(fill, ramp);
  const bias = (hash(cell.col) - 0.5) * 4;      // the whole patch leans together
  const autumn = season === 3;
  const winter = season === 0;

  const ops = [];
  const stroke = (z, d, color, width) => ops.push({ z, t: 'path', d, stroke: color, w: width });
  const body = (d, width) =>
    ops.push({ z: Z.BODY, t: 'path', d, fill, stroke: ramp.ink, w: width });
  const oval = (z, ox, oy, rx, ry, f, s, width) =>
    ops.push({ z, t: 'oval', x: ox, y: oy, rx, ry, fill: f, stroke: s, w: width });

  // --- the shadow that lifts a bush or a hedge off the paper ---------------
  if (g.kind === 'bush' || g.kind === 'hedge') {
    const d = [];
    for (let k = 0; k < 3; k++) {
      const sx = cx - halfW * 0.34 + k * halfW * 0.34 + 3;
      d.push(['M', sx, base + 3], ['L', sx + 5.5, base - 2.5]);
    }
    stroke(Z.SHADOW, d, `rgba(${ramp.inkRgb || '44,44,42'},0.14)`, 1.2);
  }

  // --- the body -----------------------------------------------------------
  let peaksOut = null;
  if (g.kind === 'tuft') {
    // a small filled clump for the blades to stand in
    const ch = h * 0.34;
    body([
      ['M', cx - halfW, base],
      ['Q', w(cx - halfW * 0.8, 1), w(base - ch * 1.6, 1), w(cx, 1), base - ch],
      ['Q', w(cx + halfW * 0.8, 1), w(base - ch * 1.6, 1), cx + halfW, base],
      ['Z'],
    ], g.width);
  } else if (g.kind === 'bush' || g.kind === 'hedge') {
    const lobes = g.kind === 'bush' ? 3 : 4;
    const shape = g.kind === 'bush'
      ? [[0.86, 0], [1, 0], [0.9, 0]]
      : [[0.82, 0], [0.97, 0], [1, 0], [0.86, 0]];
    const peaks = shape.map(([p]) => [p + (rnd() - 0.5) * 0.12, (rnd() - 0.5) * 1.6]);
    const shell = lobed(cx, base, halfW, h, lobes, peaks, w);
    body(shell.d, g.width);
    peaksOut = shell.tops.map((t, i) => {
      const b = shell.tops[i + 1];
      if (!b) return null;
      return [(t[0] + b[0]) / 2 + peaks[i][1], base - h * peaks[i][0]];
    }).filter(Boolean);
  }

  // --- blades (thin kinds) or inner strokes (filled kinds) ----------------
  const n = g.blades;
  if (g.kind === 'sprouts' || g.kind === 'tuft') {
    const foot = g.kind === 'tuft' ? base - h * 0.12 : base;
    for (let i = 0; i < n; i++) {
      const f = n === 1 ? 0.5 : i / (n - 1);
      const bx = cx - halfW * 0.8 + f * halfW * 1.6 + (rnd() - 0.5) * 2;
      const bh = h * (0.8 + rnd() * 0.4);
      const lean = bias + (rnd() - 0.5) * (5 + level);
      const turned = autumn && rnd() < 0.15;
      const b = blade(bx, foot, bh, lean, w);
      stroke(Z.DETAIL, b.d, turned ? AUTUMN_BLADE : fill, g.width);
      if (winter && i % 3 === 0) {
        oval(Z.ACCENT, b.tip[0], b.tip[1] - 0.4, 1.5, 1.5, '#FFFFFF', null, 0);
      }
    }
  } else if (peaksOut) {
    const d = [];
    const brown = [];
    for (let i = 0; i < n; i++) {
      const f = n === 1 ? 0.5 : i / (n - 1);
      const sx = cx - halfW * 0.62 + f * halfW * 1.24 + (rnd() - 0.5) * 1.6;
      const y0 = base - h * 0.06;
      const y1 = base - h * (0.42 + rnd() * 0.42);
      const lean = bias * 0.6 + (rnd() - 0.5) * 4;
      const seg = [
        ['M', w(sx, 0.5), y0],
        ['Q', w(sx + lean * 0.4, 0.8), (y0 + y1) / 2, w(sx + lean, 0.6), w(y1, 0.7)],
      ];
      (autumn && rnd() < 0.18 ? brown : d).push(...seg);
    }
    const iw = Math.max(0.9, g.width * 0.72);
    if (d.length) stroke(Z.DETAIL, d, inner, iw);
    if (brown.length) stroke(Z.DETAIL, brown, AUTUMN_BLADE, iw);

    // seed heads on the tallest lobes of a hedge
    if (g.kind === 'hedge') {
      const heads = [...peaksOut].sort((a, b) => a[1] - b[1]).slice(0, 2 + (rnd() < 0.5 ? 1 : 0));
      for (const p of heads) {
        oval(Z.ACCENT, p[0], p[1] - 2.4, 1.5, 3, mix(fill, '#FFFFFF', 0.28), ramp.ink, 0.8);
      }
    }
    // snow settles in the dips of a winter bush or hedge
    if (winter) {
      for (const p of peaksOut) {
        oval(Z.ACCENT, p[0], p[1] + 0.6, 2.6, 1.5, '#FFFFFF', null, 0);
      }
    }
  }

  // --- the best days of the year: a dandelion and a seed-head puff ---------
  if (cell.heroic && !winter) {
    const st = h * 0.9 + 8;
    for (let k = 0; k < 2; k++) {
      const dx = cx + (k ? 6 : -5.5) + (rnd() - 0.5) * 2;
      const top = base - st * (k ? 0.82 : 1);
      const hx = dx + bias * 0.4;
      stroke(Z.ACCENT, [
        ['M', w(dx, 0.6), w(base, 0.6)],
        ['Q', w((dx + hx) / 2, 0.9), w((base + top) / 2, 0.9), w(hx, 0.6), w(top, 0.6)],
      ], ramp.ink, 1);
      if (k === 0) {
        oval(Z.ACCENT, hx, top, 3.4, 3.4, DANDELION, ramp.ink, 0.9);
        const rays = [];
        for (let j = 0; j < 6; j++) {
          const a = j * 1.047 + 0.3;
          rays.push(['M', hx + Math.cos(a) * 3.4, top + Math.sin(a) * 3.4],
            ['L', hx + Math.cos(a) * 5, top + Math.sin(a) * 5]);
        }
        stroke(Z.ACCENT, rays, DANDELION, 1.1);
      } else {
        oval(Z.ACCENT, hx, top, 3.1, 3.1, FLUFF, null, 0);
        const spokes = [];
        for (let j = 0; j < 6; j++) {
          const a = j * 1.047 + 0.4;
          spokes.push(['M', hx + Math.cos(a) * 2, top + Math.sin(a) * 2],
            ['L', hx + Math.cos(a) * 4.6, top + Math.sin(a) * 4.6]);
        }
        stroke(Z.ACCENT, spokes, ramp.ink, 0.6);
      }
    }
  }

  // --- spring blossom, scattered through whatever is growing --------------
  if (season === 1) {
    for (let f = 0; f < 2; f++) {
      const fx = cx + (rnd() - 0.5) * Math.max(CELL * 0.7, halfW * 1.5);
      const fy = base - 2 - rnd() * h * 0.9;
      oval(Z.ACCENT, fx, fy, 1.6, 1.6, FLOWERS[Math.floor(rnd() * FLOWERS.length)], null, 0);
    }
  }

  return mowT > 0 ? squash(ops, cx, base, mowT) : ops;
}

/**
 * The cut: the silhouette flattens to 0.15 of its height on a 0.25s ease-out
 * and bulges 1.15x wide on the way down, then fades out over the last of it so
 * the clean GitHub tile is what is left.
 */
function squash(ops, cx, base, mowT) {
  const e = easeOut(Math.min(1, mowT));
  const sy = 1 - (1 - FLAT) * e;
  const sx = 1 + OVERSHOOT * Math.sin(Math.PI * e);
  const alpha = mowT < 0.6 ? 1 : Math.max(0, (1 - mowT) / 0.4);
  const X = (v) => cx + (v - cx) * sx;
  const Y = (v) => base + (v - base) * sy;
  return ops.map((op) => {
    if (op.t === 'oval') {
      return { ...op, x: X(op.x), y: Y(op.y), rx: op.rx * sx, ry: op.ry * sy, alpha };
    }
    const d = op.d.map((seg) => {
      const out = [seg[0]];
      for (let i = 1; i < seg.length; i += 2) out.push(X(seg[i]), Y(seg[i + 1]));
      return out;
    });
    return { ...op, d, alpha };
  });
}
